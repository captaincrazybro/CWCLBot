...
module.exports = class _User {
    ...
    constructor(id){
        ...
        if(!punishments[id]){
            punishments[id] = [];
            fs.writeFile('./js/bot/storage/punishments.json', JSON.stringify(punishments), (err) => {
        ...
        if(!economy[id]){
            ...
            fs.writeFile('./js/bot/storage/economy.json', JSON.stringify(economy), err => {
                if(err) console.log(err);
            })
        }
        fs.writeFile('./js/bot/storage/permissions.json', JSON.stringify(users), (err) => {
          if(err) console.log(err);
        })
