...
const _User = require('./js/bot/util/Constructors/_User')
const _Role = require("./js/bot/util/Constructors/_Role.js")
const _MinecraftAPI = require('./js/bot/util/Constructors/_MinecraftAPI');
const MojangAPI = require('mojang-api')
const ms = require('ms');
const Colors = require("./js/bot/util/Enums/Colors.js");
const fs = require("fs");
const _Player = require("./js/bot/util/Constructors/_Player.js");
const filter = require("./js/bot/storage/filter.json");
const _NoticeEmbed = require("./js/bot/util/Constructors/_NoticeEmbed")
const _Blacklist = require("./js/bot/util/Constructors/_Blacklist");
...
bot.commands = new Discord.Collection();  
 fs.readdir('./js/bot/commands/', (err, files) => {
 
  files.filter(f => f.split(".").length == 1).forEach((f2, i) => {
 
  fs.readdir(`./js/bot/commands/${f2}/`, (err, files) => {
    ...
    jsfile.forEach((f, i) =>{
        ...
        let props = require(`./js/bot/commands/${f2}/${f}`);
...
bot.on("ready", async () => {
    ...
    bot.user.setPresence({ game: { name: "Cake Wars" } });
});
...
bot.on('messageUpdate', (oldMessage, newMessage) => {
  ...
  if(newMessage.channel.guild.id != '665698425601392749') return;
  ...
  let embed = new Discord.RichEmbed()
    ...
    .addField("New Message", oldMessage.content)
    .addField("Channel", `<#${newMessage.channel.id}>`)
    .setTimestamp(new Date());
 
  bot.guilds.get('665698425601392749').channels.get('678679440993288263').send(embed);
});
 
bot.on("messageDelete", (message) => {

  if(message.channel.guild.id != '665698425601392749') return;
